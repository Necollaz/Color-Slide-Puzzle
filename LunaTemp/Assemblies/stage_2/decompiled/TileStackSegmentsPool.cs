...
public class TileStackSegmentsPool
{
	...
	{
		int requiredCount = Mathf.Max(1, _config.MaxStackSize);
	...
	public void UpdateSegmentsPositions(int count, Transform stackTransform, float segmentBaseLocalY, float segmentStepLocal, float gapLocal)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (!(segment == null))
			{
				...
				localPosition.x = 0f;
				localPosition.z = 0f;
				localPosition.y = segmentBaseLocalY + (float)i * (segmentStepLocal + gapLocal);
	...
	private void EnsureSegmentsCount(int requiredCount)
	{
		...
		while (_segments.Count < requiredCount)
		{
			...
			{
				newSegment.transform.SetParent(parent, false);
			}
	...
	private MeshRenderer CreateSegmentInstance()
	{
		...
		newSegmentRenderer.name = "TileSegment";
		if (newSegmentRenderer.TryGetComponent<TileStackView>(out var strayView))
		{
			strayView.enabled = false;
		}
		if (newSegmentRenderer.TryGetComponent<Collider>(out var strayCollider))
		{
