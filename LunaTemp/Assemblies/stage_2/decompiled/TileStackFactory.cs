...
public class TileStackFactory
{
	...
	public TileStackFactory(TileStackView cellStackPrefab, TileStackRoot spawnStackRootPrefab, TileStackColorsGenerator colorsGenerator, TileConfig config, GridCellsBuilder cellsBuilder, TileSpawnMatchPlanner spawnMatchPlanner, TileSpawnTopColorSelector spawnTopColorSelector, TileColorStatistics colorStatistics)
	{
	...
	public TileStackRoot CreateOrReuseSpawnStack(Transform parentTransform)
	{
		...
		stackRoot.transform.SetParent(parentTransform, false);
	...
	private void InitializeSpawnStack(TileStackView stackView)
	{
		...
		Color topColor;
		if (_spawnMatchPlanner != null && _spawnMatchPlanner.TryBuildSpawnStackColors(safeStackSize, _spawnStackColorsBuffer) && _spawnStackColorsBuffer.Count > 0)
		{
			segmentColors = _spawnStackColorsBuffer;
		}
		else if (_spawnMatchPlanner != null && _spawnTopColorSelector.TryGetHelpfulSpawnTopColor(out topColor))
		{
			...
			{
				int safeMonocolorSize = Mathf.Clamp(safeStackSize, minimumTilesPerColor, maxAdditionalTilesWithoutOverflow);
	...
	private void TryMatchRootScaleWithGrid(Transform rootTransform)
	{
		if (!(rootTransform == null))
		{
			...
			if (_isGridScaleCached)
			{
				...
				float parentX = ((Mathf.Abs(parentWorldScale.x) < 0.0001f) ? 1f : parentWorldScale.x);
				float parentY = ((Mathf.Abs(parentWorldScale.y) < 0.0001f) ? 1f : parentWorldScale.y);
				float parentZ = ((Mathf.Abs(parentWorldScale.z) < 0.0001f) ? 1f : parentWorldScale.z);
