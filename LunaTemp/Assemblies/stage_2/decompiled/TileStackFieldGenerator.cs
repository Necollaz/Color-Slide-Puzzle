...
public class TileStackFieldGenerator
{
	...
	public TileStackFieldGenerator(TileEmptyCellSelector emptyCellSelector, TileStackClusterBuilder clusterBuilder, Grid grid, TileConfig tileConfig, GridNeighborOffsetProvider gridNeighborOffset)
	{
	...
	public void TryGenerateStacks(IReadOnlyDictionary<Vector2Int, HexCellView> cellViewsByCoordinates, int desiredOccupiedCells)
	{
		if (cellViewsByCoordinates == null || cellViewsByCoordinates.Count == 0)
		{
			return;
		}
		Vector2Int emptyCellCoordinates = (EmptyCellCoordinates = _emptyCellSelector.ChooseEmptyCellClosestToCamera(cellViewsByCoordinates));
		...
		if (totalFillableCells > 0)
		{
			...
			_clusterBuilder.BuildRandomClusters(cellViewsByCoordinates, availableCoordinates, emptyCellCoordinates, targetOccupiedCells);
	...
	private void RemoveInitialNeighborTopMatches(IReadOnlyDictionary<Vector2Int, HexCellView> cells)
	{
		...
		foreach (KeyValuePair<Vector2Int, HexCellView> pair in cells)
		{
			...
			if (!(cellView == null))
			{
				...
				if (!(stackView == null) && stackView.TryGetTopColor(out var topColor) && HasNeighborWithSameTopColor(coordinates, topColor, cells, neighborOffsets) && TryPickTopColor(coordinates, cells, neighborOffsets, palette, out var newTopColor))
				{
	...
	private bool HasNeighborWithSameTopColor(Vector2Int cellCoordinates, Color topColor, IReadOnlyDictionary<Vector2Int, HexCellView> cells, Vector2Int[] neighborOffsets)
	{
	...
	private bool TryPickTopColor(Vector2Int cellCoordinates, IReadOnlyDictionary<Vector2Int, HexCellView> cells, Vector2Int[] neighborOffsets, IReadOnlyList<Color> palette, out Color resultColor)
	{
		...
		for (int i = 0; i < neighborOffsets.Length; i++)
		{
			...
			{
				TileStackView neighborStack = neighborCell.GetComponentInChildren<TileStackView>();
				if (!(neighborStack == null) && neighborStack.TryGetTopColor(out var neighborTopColor))
				{
