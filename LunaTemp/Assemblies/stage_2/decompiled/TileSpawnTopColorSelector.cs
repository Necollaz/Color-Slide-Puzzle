...
public class TileSpawnTopColorSelector
{
	...
	public TileSpawnTopColorSelector(Grid grid, GridCellsBuilder cellsBuilder, TileConfig tileConfig, TileColorStatistics colorStatistics, GridNeighborOffsetProvider gridNeighborOffset)
	{
		_grid = grid;
	...
	public bool TryGetHelpfulSpawnTopColor(out Color topColor)
	{
		...
		foreach (KeyValuePair<Vector2Int, HexCellView> pair in cells)
		{
			...
			for (int index = 0; index < neighborOffsets.Length; index++)
			{
				...
				if (matchPower > 0)
				{
					...
					int score = matchPower * 10 - remainder;
		...
		if (foundBest && bestScore > 0)
		{
			topColor = bestColor;
			return true;
		}
		if (TryPickColorByRemainder(out var fallbackColor))
		{
	...
	private int CountSameTopNeighbors(Vector2Int cellCoordinates, Color topColor, IReadOnlyDictionary<Vector2Int, HexCellView> cells, Vector2Int[] neighborOffsets)
	{
		int count = 0;
		for (int index = 0; index < neighborOffsets.Length; index++)
		{
			...
			{
				TileStackView neighborStack = neighborCell.GetComponentInChildren<TileStackView>();
				if (!(neighborStack == null) && !neighborStack.IsEmpty && neighborStack.TryGetTopColor(out var neighborTopColor) && neighborTopColor == topColor)
				{
					count++;
				}
			}
