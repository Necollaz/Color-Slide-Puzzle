...
public class TileStackColorsGenerator
{
	...
	public IReadOnlyList<Color> BuildFieldStackColors(int stackSize)
	{
		...
		int minimumTilesPerColor = Mathf.Max(3, _config.MinTilesPerColorBlock);
		...
		int distinctColorsCount = _colorBlocksBuilder.ChooseDistinctColorsCount(maxDistinctColors, safeStackSize, minimumTilesPerColor);
		...
		{
			attemptsLeft--;
			Color candidate2 = paletteColors[Random.Range(0, paletteCount)];
			if (!_distinctColorsBuffer.Contains(candidate2))
			{
				_distinctColorsBuffer.Add(candidate2);
			}
		}
		...
		_colorBlocksBuilder.BuildStackFromDistinctColors(_distinctColorsBuffer, safeStackSize, minimumTilesPerColor, _stackColorsBuffer);
		return _stackColorsBuffer;
	}
