...
public class TileSpawnMatchPlanner
{
	...
	public TileSpawnMatchPlanner(TileConfig tileConfig, GridCellsBuilder cellsBuilder, TileColorStatistics colorStatistics, TileSpawnUsefulTopColorsCollector usefulTopColorsCollector, TileColorRemainderSorter colorRemainderSorter, TileSpawnStackFromColorsBuilder stackFromColorsBuilder)
	{
	...
	public bool TryBuildSpawnStackColors(int stackSize, List<Color> resultColors)
	{
		...
		if (_usefulTopColorsList.Count > 0)
		{
			...
			if (_stackFromColorsBuilder.TryBuildFinishingStack(_usefulTopColorsList, stackSize, maxStackThreshold, resultColors))
			{
				return true;
			}
			if (_stackFromColorsBuilder.TryBuildHelpfulSpawnStackFromColors(_usefulTopColorsList, stackSize, minimumTilesPerColor, maxStackThreshold, resultColors))
			{
		...
		if (_stackFromColorsBuilder.TryBuildHelpfulSpawnStackFromColors(_paletteBuffer, stackSize, minimumTilesPerColor, maxStackThreshold, resultColors))
		{
			return resultColors.Count > 0;
		}
		return false;
