...
public class TileSpawnStackFromColorsBuilder
{
	...
	public bool TryBuildHelpfulSpawnStackFromColors(IReadOnlyList<Color> sourceColors, int stackSize, int minimumTilesPerColor, int maxStackThreshold, List<Color> resultColors)
	{
		resultColors.Clear();
		...
		for (int index4 = 0; index4 < effectiveMaxDistinctColors; index4++)
		{
			Color color = chosenColors[index4];
			...
			if (maximalTilesForThisColor < minimalTilesForThisColor)
			{
				...
				int safeMaxForThisColor = remainingTiles - (remainingColors - 1) * minimumTilesPerColor;
				maximalTilesForThisColor = Mathf.Min(maxTilesPerColorSpawn, Mathf.Max(minimalTilesForThisColor, safeMaxForThisColor));
			}
			int targetTilesForThisColor = Mathf.Clamp(neededToThreshold, minimalTilesForThisColor, maximalTilesForThisColor);
			if (remainingColors == 1)
			{
				targetTilesForThisColor = Mathf.Clamp(remainingTiles, minimalTilesForThisColor, maxTilesPerColorSpawn);
			}
			targetTilesForThisColor = Mathf.Clamp(targetTilesForThisColor, minimalTilesForThisColor, maxTilesPerColorSpawn);
		...
		while (remainingTiles > 0 && iterationsLeft > 0)
		{
			iterationsLeft--;
			bool addedAny = false;
			for (int index3 = 0; index3 < colorBlocks.Count; index3++)
			{
				if (remainingTiles <= 0)
				{
					break;
				}
				TileColorBlock block2 = colorBlocks[index3];
				int canTake = _tileConfig.MaxTilesPerColorSpawn - block2.Count;
				if (canTake > 0)
				{
		...
		for (int index2 = 0; index2 < colorBlocks.Count; index2++)
		{
			TileColorBlock block = colorBlocks[index2];
	...
	public bool TryBuildFinishingStack(IReadOnlyList<Color> colors, int stackSize, int maxStackThreshold, List<Color> resultColors)
	{
		resultColors.Clear();
		...
		for (int index = 0; index < colors.Count; index++)
		{
			...
			if (tilesNeededToThreshold <= stackSize && tilesNeededToThreshold <= _tileConfig.MaxTilesPerColorSpawn)
			{
