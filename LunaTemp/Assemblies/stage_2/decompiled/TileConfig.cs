...
public class TileConfig : ScriptableObject
{
	...
	[SerializeField]
	[Min(1f)]
	private int _minGeneratedStackSize = 3;

	[SerializeField]
	[Min(1f)]
	private int _maxGeneratedStackSize = 8;

	[Header("Cluster generation")]
	[SerializeField]
	[Min(1f)]
	private int _minClusterSize = 2;

	[SerializeField]
	[Min(1f)]
	private int _maxClusterSize = 4;

	[Header("Color blocks per stack")]
	[SerializeField]
	[Min(1f)]
	private int _minColorBlocksPerStack = 1;

	[SerializeField]
	[Min(1f)]
	private int _maxColorBlocksPerStack = 3;

	[SerializeField]
	[Min(1f)]
	private int _maxColorPickAttempts = 8;

	[SerializeField]
	[Min(1f)]
	private int _minTilesPerColorBlock = 1;

	[Header("Spawn rules per color")]
	[SerializeField]
	[Min(1f)]
	private int _minTilesPerColorSpawn = 3;

	[SerializeField]
	[Min(1f)]
	private int _maxTilesPerColorSpawn = 9;

	[SerializeField]
	[Min(1f)]
	private int _maxStackDistributionIterations = 64;

	[SerializeField]
	[Min(1f)]
	private int _maxColorsPerSpawnStack = 3;

	[Header("Color blocks spawn weights (%)")]
	[SerializeField]
	[Range(0f, 100f)]
	private int _oneColorStackWeightPercent = 10;

	[SerializeField]
	[Range(0f, 100f)]
	private int _twoColorsStackWeightPercent = 60;

	[SerializeField]
	[Range(0f, 100f)]
	private int _threeColorsStackWeightPercent = 30;

	[Header("Visual size relative to HexCell (field stacks)")]
	[SerializeField]
	[Range(0.1f, 1f)]
	private float _xzScaleFactor = 0.9f;

	[SerializeField]
	[Range(0.1f, 1f)]
	private float _stackHeight = 0.3f;
	...
	[SerializeField]
	[Range(0.1f, 1f)]
	private float _spawnXzWorldSize = 0.5f;

	[SerializeField]
	[Range(0.1f, 1f)]
	private float _spawnStackWorldHeight = 1f;

	[SerializeField]
	private float _spawnYOffset = 0.05f;

	[Header("Visual stack segments")]
	[SerializeField]
	[Min(0.001f)]
	private float _segmentHeight = 0.04f;

	[SerializeField]
	[Min(0f)]
	private float _segmentGap = 0.005f;

	[Header("Effects")]
	[SerializeField]
	private ParticleSystem _tileClearEffectPrefab;

	[SerializeField]
	private ParticleSystem _stackClearEffectPrefab;

	[SerializeField]
	[Min(0f)]
	private int _tileClearEffectPrewarmCount = 20;

	[SerializeField]
	[Min(0f)]
	private int _stackClearEffectPrewarmCount = 5;
