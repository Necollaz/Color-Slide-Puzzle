...
public class TileStackClusterBuilder
{
    ...
    public void BuildRandomClusters(IReadOnlyDictionary<Vector2Int, HexCellView> cellViewsByCoordinates,
        HashSet<Vector2Int> availableCoordinates, Vector2Int emptyCellCoordinates, int targetOccupiedCells)
    {
        ...
        int remainingCellsToFill = Mathf.Clamp(targetOccupiedCells, MIN_TARGET_OCCUPIED_CELLS, maxPossibleCells);

        Vector2Int[] neighborOffsets = _gridNeighborOffset.GetNeighborOffsets(_grid);
        
        int minClusterSize = Mathf.Max(MIN_CLUSTER_SIZE_VALUE, _config.MinClusterSize);
        ...
        while (availableCoordinates.Count > 0 && remainingCellsToFill > 0)
        {
            ...
            int targetClusterSize = GetTargetClusterSize(minClusterSize, maxClusterSize, remainingCellsToFill,
                availableCoordinates.Count);
            int paintedInCluster = GrowCluster(seedCoordinates, targetClusterSize, availableCoordinates, 
                cellViewsByCoordinates, neighborOffsets);
    ...
    private int GrowCluster(Vector2Int seedCoordinates, int targetClusterSize, HashSet<Vector2Int> availableCoordinates,
        IReadOnlyDictionary<Vector2Int, HexCellView> cellViewsByCoordinates, Vector2Int[] neighborOffsets)
    {
    ...
    private int GetTargetClusterSize(int minClusterSize, int maxClusterSize, int remainingCellsToFill,
        int availableCoordinatesCount)
    {
    ...
    private void EnsureExactOccupiedCount(IReadOnlyDictionary<Vector2Int, HexCellView> cellViewsByCoordinates,
        Vector2Int emptyCellCoordinates, int targetOccupiedCells)
    {
        ...
        foreach (KeyValuePair<Vector2Int, HexCellView> pair in cellViewsByCoordinates)
        {
            ...
            if (_stackFactory.HasActiveStack(cellView))
                ...
                _freeCoordinatesBuffer.Add(coordinates);
        }
        ...
        while (remainingToFill > 0 && _freeCoordinatesBuffer.Count > 0)
        {
            ...
            if (!cellViewsByCoordinates.TryGetValue(coordinates, out HexCellView cellView) || cellView == null)
                continue;
