...
public class TileStackRuntime
{
    ...
    public int CountTopTilesOfColor(Color color) => _model.CountTopTilesOfColor(color);
    ...
    public bool TryGetBottomColorIndex(Color color, out int indexFromBottom) => 
        _model.TryGetBottomColorIndex(color, out indexFromBottom);

    public bool TryGetTopColor(out Color color) => _model.TryGetTopColor(out color);

    public bool TryGetCompletedColor(out Color color, out int completedCount) =>
        _model.TryGetCompletedColor(out color, out completedCount);
    ...
    public void Initialize(IReadOnlyList<Color> segmentColors)
    {
        ...
        ApplyModelToView(fullRebuildStackSize: true);
    }
